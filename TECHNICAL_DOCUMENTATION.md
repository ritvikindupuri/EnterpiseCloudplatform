...
**1. Download Mining Software**: Utilizing `curl`, the script pulls a live archive of XMRig, a legitimate Monero miner heavily favored by malicious actors, storing it in `/tmp`. GuardDuty natively flags this download pattern.

**2. Connect to Mining Pools**: Utilizing `netcat` (`nc -zv`), the script systematically attempts to open TCP connections against prominent, known cryptocurrency mining pools on their default ports. Because AWS GuardDuty maintains active threat intelligence on these exact domains, it flags these DNS requests and triggers a `CryptoCurrency:EC2/BitcoinTool.B!DNS` finding.

**3. High CPU Usage Pattern**: To faithfully simulate the compute drain of a cryptocurrency miner, the script launches 4 background subshells (`&`) running infinite while-loops calculating pi (`4*a(1)`) to 5000 decimal places via the `bc` utility. This immediately pins the `t3.medium` vCPUs to 80-90% utilization, creating a highly visible anomaly on CloudWatch metrics graphs.

**4. Persistence Mechanism**: A classic malware technique is utilized where the script writes a new `crontab` entry specifically executing the miner script upon server reboot (`@reboot`), ensuring the attack survives a naive restart.

**5. Process Hiding**: Attempting to hide in plain sight, the bash process is copied and renamed to identically mimic a kernel worker thread (`[kworker/0:1]`), a primitive form of process masquerading intended to fool casual administrator glances in `top` or `ps` outputs.
...
**1. S3 Bucket Enumeration**: The script initiates reconnaissance using the AWS CLI `aws s3 ls` command to map out all accessible buckets within the AWS account, creating an immediate, visible anomaly inside CloudTrail's API logging.

**2. Bucket Content Access**: Using an `awk` iterator, the script parses the previous reconnaissance list and automatically runs a recursive list command (`aws s3 ls s3://$bucket --recursive`) against every discovered bucket. Attempting to bulk map an entire account's storage tier triggers GuardDuty's unusual access pattern heuristics and leaves a permanent forensic trail inside CloudTrail.

**3. Large Data Download Simulation**: To simulate the actual exfiltration payload and network spike, the script utilizes the Linux `dd` utility, reading directly from `/dev/urandom` to build a 100MB dummy file named `sensitive-data.bin`. The system relies on this type of heavy storage IO simulation to subsequently trigger GuardDuty's `Exfiltration:S3/ObjectRead.Unusual` rule.

**4. External Exfiltration Attempts**: The script simulates shipping the data offsite by executing `nslookup` queries against known malicious/suspicious domains (e.g., `attacker-c2.net`). The VPC Flow Logs inherently capture this outbound connection request, immediately triggering GuardDuty's threat intelligence alarms.

**5. Unusual API Call Pattern**: Rounding out the reconnaissance phase, the instance systematically invokes IAM user listings, EC2 queries, and CloudTrail descriptions. For a `t3.small` instance completely uninvolved in systems administration, these `list` and `describe` API requests are exceptionally anomalous and purposefully crafted to be flagged by our secondary ML Lambda sequence detector.
...
**1. IAM Permission Enumeration**: By explicitly calling `aws iam list-users`, `list-roles`, and `list-policies`, the instance aggressively attempts to map out the entire permission boundary of the AWS account. CloudTrail acts as an immutable ledger, logging every unauthorized `AccessDenied` exception.

**2. Access Key Creation Attempt**: The script invokes `aws iam create-access-key` targeting an `admin` user. While it will generally fail due to our least-privilege setup, the sheer attempt to generate hardcoded credentials for persistence triggers severe GuardDuty alerts like `PenTest:IAMUser/KaliLinux` as it matches common offensive tooling behavior.

**3. Admin Policy Attachment Attempt**: Simulating an active privilege escalation, the script attempts an `aws iam attach-user-policy` command to forcefully bind the full `AdministratorAccess` ARN to a standard `test-user`. Security Hub constantly parses IAM policy modifications and natively flags this exact activity alongside CloudTrail.

**4. Role Assumption Attempts**: Utilizing an iterative loop against previously enumerated roles, the script invokes the `aws sts assume-role` API, appending a custom `attack-session` name to the request. This represents an attacker dynamically testing for overly permissive trust policies to pivot laterally inside the AWS environment. GuardDuty evaluates these programmatic sequences as highly unusual role assumption patterns.

**5. Security Group Modification Attempt**: As a precursor to opening network backdoors or allowing lateral SSH movements, the script calls `aws ec2 describe-security-groups`. This provides the attacker the network topology needed to launch subsequent egress changes.

**6. Backdoor User Creation**: Finally, the script attempts `aws iam create-user` to silently build an invisible `backup-admin` user. Like all earlier phases, this generates direct `AccessDenied` footprints inside CloudTrail which our custom ML model ingests and matches against its malicious IAM pattern library to trigger a confidence alert.
